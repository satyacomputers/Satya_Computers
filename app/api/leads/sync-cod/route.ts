...
export async function POST(req: Request) {
  try {
    ...
    const { customerName, phone, remarks, totalAmount = 0, address = 'Store / Phone Order', password } = body;

    // Verify passcode
    if (password !== 'Satya@2323') {
      return NextResponse.json({ error: 'Unauthorized: Invalid Security Password' }, { status: 401 });
    }
    ...
    await client.execute({
      ...
      args: [
        ...
        productsJson,
        totalAmount,
        remarks || 'Synced from Standalone Leads Dashboard'
      ]
    });
